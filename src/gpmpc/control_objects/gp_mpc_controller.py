...
class GpMpcController(BaseControllerObject):
    ...
    def __init__(
        ...
        self.weight_matrix_cost_terminal = torch.diag(
        ...
        self.max_change_action_norm = np.array(
            params_dict["controller"]["max_change_action_norm"]
        )
        self.clip_lower_bound_cost_to_0 = params_dict["controller"][
            "clip_lower_bound_cost_to_0"
        ]
        ...
        if self.limit_action_change:
            self.bounds = [
                (
                    -self.max_change_action_norm[idx_action],
                    self.max_change_action_norm[idx_action],
                )
                for idx_action in range(self.action_space.shape[0])
            ] * self.len_horizon
            self.actions_pred_previous_iter = np.dot(
                np.expand_dims(
                    np.random.uniform(low=-1, high=1, size=(self.len_horizon)), 1
                ),
                np.expand_dims(self.max_change_action_norm, 0),
            )
        else:
            self.bounds = [(0, 1)] * self.num_actions * self.len_horizon
            self.actions_pred_previous_iter = np.random.uniform(
    ...
    def preprocess_params(params_dict):
        ...
        params_dict["controller"]["target_state_norm"] = torch.Tensor(
            params_dict["controller"]["target_state_norm"]
        )
        params_dict["controller"]["weight_state"] = torch.Tensor(
            params_dict["controller"]["weight_state"]
        )
        params_dict["controller"]["weight_state_terminal"] = torch.Tensor(
            params_dict["controller"]["weight_state_terminal"]
        )
        params_dict["controller"]["target_action_norm"] = torch.Tensor(
            params_dict["controller"]["target_action_norm"]
        )
        ...
        params_dict["memory"]["min_error_prediction_state_for_memory"] = torch.Tensor(
            params_dict["memory"]["min_error_prediction_state_for_memory"]
        )
        params_dict["memory"]["min_prediction_state_std_for_memory"] = torch.Tensor(
    ...
    def to_normed_obs_tensor(self, obs):
        ...
        """
        state_mu_norm = torch.Tensor(
            (obs - self.obs_space.low) / (self.obs_space.high - self.obs_space.low)
        )
    ...
    def to_normed_var_tensor(self, obs_var):
        ...
        obs_var_norm = torch.Tensor(
            obs_var_norm / (self.obs_space.high - self.obs_space.low).T
        )
        return obs_var_norm
    ...
    def calculate_factorizations(x, y, models):
        ...
        L = torch.linalg.cholesky(
            K
            + torch.stack([model.likelihood.noise for model in models])[:, None]
            * batched_eye
        )
    ...
    def predict_next_state_change(self, state_mu, state_var, iK, beta):
        ...
        inp = (self.x[self.idxs_mem_gp[: beta.shape[1]]] - state_mu)[None, :, :].repeat(
            [self.num_states, 1, 1]
        )
        lengthscales = torch.stack(
            [model.covar_module.base_kernel.lengthscale[0] for model in self.models]
        )
        ...
        V = (
            torch.matmul(torch.transpose(tiL.conj(), -1, -2), lb[:, :, None])[..., 0]
            * c[:, None]
        )
        ...
        maha = (
            -2 * torch.matmul(torch.matmul(X, Q), torch.transpose(X2.conj(), -1, -2))
            + Xs[:, :, :, None]
            + X2s[:, :, None, :]
        )
        ...
        S = (temp @ beta[None, :, :, None].repeat([self.num_states, 1, 1, 1]))[
            :, :, 0, 0
        ]

        diagL = torch.Tensor.permute(
            torch.diagonal(
                torch.Tensor.permute(L, dims=(3, 2, 1, 0)), dim1=-2, dim2=-1
            ),
            dims=(2, 1, 0),
        )
    ...
    def predict_trajectory(self, actions, obs_mu, obs_var, iK, beta):
        ...
        states_var_pred = torch.empty(
            (self.len_horizon + 1, self.num_states, self.num_states)
        )
        ...
        for idx_time in range(1, self.len_horizon + 1):
            ...
            input_mean[self.num_states : (self.num_states + self.num_actions)] = (
                actions[idx_time - 1]
            )
            ...
            states_var_pred[idx_time] = (
                state_change_var
                + states_var_pred[idx_time - 1]
    ...
    def compute_mean_lcb_trajectory(self, actions, obs_mu, obs_var, iK, beta):
        ...
        gradients_dcost_dactions = torch.autograd.grad(
            mean_cost_traj_lcb, actions, retain_graph=False
        )[0]
        ...
        with torch.no_grad():
            ...
            init_actions_optim = np.concatenate(
                (
                    ...
                    np.expand_dims(self.actions_pred_previous_iter[-1], 0),
                ),
                axis=0,
            )
            ...
            if self.limit_action_change:
                ...
                init_actions_optim_absolute = np.cumsum(
                    init_actions_optim_absolute, axis=0
                )
                if np.logical_or(
                    np.any(init_actions_optim_absolute > 1),
                    np.any(init_actions_optim_absolute < 0),
                ):
                    for idx_time in range(1, len(init_actions_optim)):
                        init_actions_optim_absolute[idx_time] = (
                            init_actions_optim_absolute[idx_time - 1]
                            + init_actions_optim[idx_time]
                        )
                        indexes_above_1 = np.nonzero(
                            init_actions_optim_absolute[idx_time] > 1
                        )[0]
                        indexes_under_0 = np.nonzero(
                            init_actions_optim_absolute[idx_time] < 0
                        )[0]
                        init_actions_optim[idx_time][indexes_above_1] = (
                            1
                            - init_actions_optim_absolute[idx_time - 1][indexes_above_1]
                        )
    ...
    def add_memory(
        ...
        if len(self.x) < (self.len_mem + 1):
            self.x = torch.cat(
                self.x, torch.empty(self.points_add_mem_when_full, self.x.shape[1])
            )
            self.y = torch.cat(
                self.y, torch.empty(self.points_add_mem_when_full, self.y.shape[1])
            )
            self.rewards = torch.cat(
                self.rewards, torch.empty(self.points_add_mem_when_full)
            )

        self.x[self.len_mem, : (obs_norm.shape[0] + action_norm.shape[0])] = torch.cat(
            (obs_norm, action_norm)
        )[None]
    ...
    def check_and_close_processes(self):
        ...
        if (
            ...
            and not self.p_train._closed
            and not (self.p_train.is_alive())
        ):
