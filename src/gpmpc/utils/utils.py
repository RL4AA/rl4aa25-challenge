...
def create_models(
    ...
    Args:
        ...
        train_targets (torch.Tensor or None): target values in the memory of the
        gps. Represent the change in state values
        params (dict or list of dict): Value of the hyper-parameters of the
        gaussian processes. Dict type is used to create the models with init
        values from the json file. List of dict type is used to create the models
        with exported parameters, such as in the parallel training process.
        constraints_gp (dict): See the ReadMe about parameters for information
        about keys.
        num_models (int or None): Must be provided when `train_inputs` or
        `train_targets` are None. The number of models should be equal to the
        dimension of state, so that the transition for each state can be
        predicted with a different gp. Default=None
        num_inputs (int or None): Must be provided when `train_inputs` or
        `train_targets` are None. The number of inputs should be equal to the
        sum of the dimension of state and dimension of action. Default=None
        include_time (bool): If True, gp will have one additional input
        corresponding to the time of the observation. This is usefull if the
        env change with time, as more recent points will be trusted more than
        past points (time closer to the point to make inference at).

    Returns:
        models (list of gpytorch.models.ExactGP): models containing the parameters,
    ...
    if train_inputs is not None and train_targets is not None:
        ...
        if num_models is None or num_inputs is None:
            raise (
                ValueError(
                    "If train_inputs or train_targets are None, num_models and"
                    + " num_inputs must be defined"
                )
            )
        else:
            models = [
...
def init_visu_and_folders(env, num_steps, params_controller_dict):
    ...
    Args:
            ...
            num_steps (int): number of steps during which the action is maintained
            constant
            env_str (str): string that describes the env name
            params_general (dict): general parameters (see parameters.md for more info)
            params_controller_dict (dict): controller parameters
            (see parameters.md for more info)

        Returns:
            live_plot_obj (object): object used to visualize the control and
            observation evolution in a 2d graph
    """
...
class LivePlotSequential:
    ...
    def update(self, obs, action, cost, info_dict=None):
        ...
        if info_dict is not None:
            ...
            # if num_repeat_action is not 1,
            # the control do not happen at each iteration,
            # we must select the last index where the control happened
            # as the start of the prediction horizon
